...
__global__ void _GidToLidKernel(IdType *global_ids, size_t size,
                                IdType *min_vids, int rank) {
...
void ConvertGidToLid(IdArray global_ids, IdArray min_vids, int rank) {
  ...
  _GidToLidKernel<<<BLOCK_NUM, BLOCK_SIZE, 0, thr_entry->stream>>>(
      global_ids_ptr, global_ids->shape[0], min_vids_ptr, rank);
  CUDACHECK(cudaGetLastError());
}

__global__ void _LidToGidKernel(IdType *local_ids, size_t size,
...
void ConvertLidToGid(IdArray local_ids, IdArray global_nid_map) {
  ...
  auto *thr_entry = CUDAThreadEntry::ThreadLocal();
  _LidToGidKernel<<<BLOCK_NUM, BLOCK_SIZE, 0, thr_entry->stream>>>(
      local_ids_ptr, local_ids->shape[0], global_nid_map_ptr);
}

__global__ void _CountDeviceVerticesKernel(int device_cnt,
...
std::tuple<IdArray, IdArray, IdArray, IdArray>
Partition(IdArray seeds, IdArray min_vids, int world_size) {
...
void Cluster(int rank, IdArray seeds, IdArray min_vids, int world_size,
...
__global__ void _CSRRowWiseSampleReplaceKernel(
    const uint64_t rand_seed, int num_picks, IdType num_rows, IdType *in_rows,
    IdType *in_ptr, IdType *in_index, IdType *out_ptr, IdType *out_index) {
...
__global__ void _CSRRowWiseSampleReplaceKernelV2(
    const uint64_t rand_seed, int num_picks, IdType num_rows, IdType *in_rows,
    IdType *in_ptr, IdType *in_index, IdType *uva_in_ptr, IdType *uva_in_index,
    IdType *adj_pos_map, IdType *out_ptr, IdType *out_index) {
  assert(blockDim.x == WARP_SIZE);
  ...
  while (out_row < last_row) {
    ...
    const int64_t out_row_start = out_ptr[out_row];
    const int64_t deg =
        (on_dev ? in_ptr[pos + 1] : uva_in_ptr[pos + 1]) - in_row_start;
...
__global__ void _CSRRowWiseSampleReplaceKernelWeightV2(
    const uint64_t rand_seed, int num_picks, IdType num_rows, IdType *in_rows,
    IdType *in_ptr, IdType *in_index, IdType *uva_in_ptr, IdType *uva_in_index,
    IdType *adj_pos_map, IdType *out_ptr, IdType *out_index, uint32_t *weight) {
  assert(blockDim.x == WARP_SIZE);
  ...
  while (out_row < last_row) {
    ...
    if (deg > 0) {
      ...
      for (int idx = threadIdx.x; idx < num_picks; idx += blockDim.x) {
        ...
        const int64_t edge =
            binarySearch(weight, in_row_start, in_row_start + deg - 1, val);
        const int64_t out_idx = out_row_start + idx;
        out_index[out_idx] = index[edge];
      }
    }
...
__global__ void _CSRRowWiseSampleReplaceKernelV2Profile(
    ...
    IdType *in_ptr, IdType *in_index, IdType *uva_in_ptr, IdType *uva_in_index,
    IdType *adj_pos_map, IdType *out_ptr, IdType *out_index,
    IdType *profile_sampled_index, IdType *profile_degree) {
  assert(blockDim.x == WARP_SIZE);
  ...
  while (out_row < last_row) {
    ...
    if (deg > 0) {
      ...
      for (int idx = threadIdx.x; idx < num_picks; idx += blockDim.x) {
        ...
        out_index[out_idx] = index[edge];
        if (on_dev) {
          profile_sampled_index[out_row * num_picks + idx] =
              edge + in_row_start;
        } else {
          profile_sampled_index[out_row * num_picks + idx] =
              ENCODE_ID(edge + in_row_start);
        }
      }
    }
    out_row += BLOCK_WARPS;
...
__global__ void _CSRRowWiseSampleReplaceKernelWeightV2Profile(
    ...
    IdType *in_ptr, IdType *in_index, IdType *uva_in_ptr, IdType *uva_in_index,
    IdType *adj_pos_map, IdType *out_ptr, IdType *out_index,
    IdType *profile_sampled_index, uint32_t *weight) {
  assert(blockDim.x == WARP_SIZE);
...
IdArray SampleNeighbors(IdArray frontier, int fanout,
                        IdArray weight = aten::NullArray(), bool bias = false) {
  ...
  if (grid.x > 0) {
    if (!ds_context->enable_profiler) {
      if (bias) {
        _CSRRowWiseSampleReplaceKernelWeightV2<BLOCK_WARPS, TILE_SIZE>
            <<<grid, block, 0, thr_entry->stream>>>(
                random_seed, fanout, n_frontier, frontier.Ptr<IdType>(),
                dev_csr.indptr.Ptr<IdType>(), dev_csr.indices.Ptr<IdType>(),
                uva_csr.indptr.Ptr<IdType>(), uva_csr.indices.Ptr<IdType>(),
                ds_context->adj_pos_map.Ptr<IdType>(),
                edge_offset.Ptr<IdType>(), neighbors.Ptr<IdType>(),
                weight.Ptr<uint32_t>());
      } else {
        _CSRRowWiseSampleReplaceKernelV2<BLOCK_WARPS, TILE_SIZE>
            <<<grid, block, 0, thr_entry->stream>>>(
                random_seed, fanout, n_frontier, frontier.Ptr<IdType>(),
                dev_csr.indptr.Ptr<IdType>(), dev_csr.indices.Ptr<IdType>(),
                uva_csr.indptr.Ptr<IdType>(), uva_csr.indices.Ptr<IdType>(),
      ...
      auto profile_sampled_index =
          IdArray::Empty({n_frontier * fanout}, frontier->dtype, dgl_ctx);
      auto profile_degree =
          IdArray::Empty({n_frontier}, frontier->dtype, dgl_ctx);
      if (bias) {
        _CSRRowWiseSampleReplaceKernelWeightV2Profile<BLOCK_WARPS, TILE_SIZE>
            <<<grid, block, 0, thr_entry->stream>>>(
                random_seed, fanout, n_frontier, frontier.Ptr<IdType>(),
                dev_csr.indptr.Ptr<IdType>(), dev_csr.indices.Ptr<IdType>(),
                uva_csr.indptr.Ptr<IdType>(), uva_csr.indices.Ptr<IdType>(),
        ...
        _CSRRowWiseSampleReplaceKernelV2Profile<BLOCK_WARPS, TILE_SIZE>
            <<<grid, block, 0, thr_entry->stream>>>(
                random_seed, fanout, n_frontier, frontier.Ptr<IdType>(),
                dev_csr.indptr.Ptr<IdType>(), dev_csr.indices.Ptr<IdType>(),
                uva_csr.indptr.Ptr<IdType>(), uva_csr.indices.Ptr<IdType>(),
      ...
      ds_context->profiler->UpdateDSSamplingLocalCount(profile_sampled_index,
                                                       profile_degree, fanout);
    }
  }
...
IdArray SampleNeighborsV2(IdArray frontier, CSRMatrix csr_mat, int fanout) {
  if (frontier->shape[0] == 0) {
    ...
    auto coo = CSRRowWiseSampling(csr_mat, frontier, fanout,
                                  NullArray(frontier->dtype, frontier->ctx));
    return coo.col;
  }
}

template <class T>
__global__ void _RemapKernel(T *dst, T *src, IdType *index, int size,
                             int fanout) {
...
IdArray Remap(IdArray neighbors, IdArray index, int fanout) {
  ...
  IdArray ret =
      IdArray::Empty({neighbors->shape[0]}, neighbors->dtype, neighbors->ctx);
  int n_threads = 512;
  int n_blocks = 128;
  auto *thr_entry = CUDAThreadEntry::ThreadLocal();
  if (neighbors->dtype.bits == 64) {
    _RemapKernel<<<n_blocks, n_threads, 0, thr_entry->stream>>>(
        ret.Ptr<IdType>(), neighbors.Ptr<IdType>(), index.Ptr<IdType>(),
        index->shape[0], fanout);
  } else {
    _RemapKernel<<<n_blocks, n_threads, 0, thr_entry->stream>>>(
        ret.Ptr<DataType>(), neighbors.Ptr<DataType>(), index.Ptr<IdType>(),
        index->shape[0], fanout);
  }
  return ret;
}
...
__global__ void _CSRRowWiseReplicateKernel(IdType size, IdType *src,
                                           IdType *dst, int fanout) {
...
void Replicate(IdArray src, IdArray *dst, int fanout) {
  ...
  *dst = IdArray::Empty({n_seeds * fanout}, src->dtype, dgl_ctx);
  // *dst = MemoryManager::Global()->Empty("DESTINATION", {n_seeds * fanout},
  // src->dtype, dgl_ctx);
...
void SampleNeighborsUVA(IdArray frontier, IdArray row_idx, CSRMatrix csr_mat,
                        int fanout, IdArray *neighbors, IdArray *edges) {
  auto dgl_ctx = frontier->ctx;
  int n_frontier = frontier->shape[0];
  // IdArray edge_offset = IdArray::Empty({n_frontier + 1}, frontier->dtype,
  // dgl_ctx);
  ...
  if (grid.x > 0) {
    if (!DSContext::Global()->enable_profiler) {
      _CSRRowWiseSampleReplaceKernel<BLOCK_WARPS, TILE_SIZE>
          <<<grid, block, 0, thr_entry->stream>>>(
              random_seed, fanout, n_frontier, frontier.Ptr<IdType>(),
              row_idx.Ptr<IdType>(), csr_mat.indices.Ptr<IdType>(),
              edge_offset.Ptr<IdType>(), neighbors->Ptr<IdType>());
    } else {
      auto profile_sampled_index =
          IdArray::Empty({n_frontier * fanout}, frontier->dtype, dgl_ctx);
      _CSRRowWiseSampleReplaceKernelUVAProfile<BLOCK_WARPS, TILE_SIZE>
          <<<grid, block, 0, thr_entry->stream>>>(
              random_seed, fanout, n_frontier, frontier.Ptr<IdType>(),
              row_idx.Ptr<IdType>(), csr_mat.indices.Ptr<IdType>(),
              edge_offset.Ptr<IdType>(), neighbors->Ptr<IdType>(),
              profile_sampled_index.Ptr<IdType>());
      CUDACHECK(cudaStreamSynchronize(thr_entry->stream));
      DSContext::Global()->profiler->UpdateUVASamplingCount(
          profile_sampled_index, fanout);
    }
  }
}

template <int BLOCK_ROWS>
__global__ void _CSRRowWiseLoadSubtensorKernel(IdType dim, IdType num_frontier,
                                               IdType *frontier,
                                               DataType *features,
...
__global__ void _CSRRowWiseLoadSubtensorAlignedKernel(
    IdType dim, IdType size, IdType *index, IdType *input_mapping,
    IdType *output_mapping, DataType *input_table, DataType *output_table) {
  IdType out_row = blockIdx.x * blockDim.y + threadIdx.y;
  while (out_row < size) {
    const IdType row = index ? index[out_row] : out_row;
    const IdType origin_in_row_start =
        input_mapping ? input_mapping[row] * dim : row * dim;
    const IdType out_row_start =
        output_mapping ? output_mapping[out_row] * dim : out_row * dim;
...
void IndexSelect(IdType size, IdArray index, IdArray input_table,
                 IdArray output_table, int feat_dim, IdArray input_mapping,
                 IdArray output_mapping, cudaStream_t stream) {
  if (stream == 0) {
    stream = CUDAThreadEntry::ThreadLocal()->stream;
  }
  // *features_to_send = IdArray::Empty({n_frontier * dim}, features->dtype,
  // dgl_ctx);
  ...
  IdType *input_mapping_ptr =
      IsNullArray(input_mapping) ? nullptr : input_mapping.Ptr<IdType>();
  IdType *output_mapping_ptr =
      IsNullArray(output_mapping) ? nullptr : output_mapping.Ptr<IdType>();
  if (grid.x > 0) {
    _CSRRowWiseLoadSubtensorAlignedKernelV2<BLOCK_ROWS>
        <<<grid, block, 0, stream>>>(
            feat_dim, size, index_ptr, input_mapping_ptr, output_mapping_ptr,
            input_table.Ptr<DataType>(), output_table.Ptr<DataType>());
    CUDACHECK(cudaGetLastError());
  }
}

void IndexSelectUVA(IdType size, IdArray index, IdArray input_table,
                    IdArray output_table, int feat_dim, IdArray input_mapping,
                    IdArray output_mapping, cudaStream_t stream) {
  if (stream == 0) {
    stream = CUDAThreadEntry::ThreadLocal()->stream;
...
// void IndexSelectUVA(IdType size, IdArray index, IdArray input_table, IdArray
// output_table, int feat_dim, IdArray input_mapping, IdArray output_mapping,
// cudaStream_t stream) {
//   if(stream == 0) {
//     stream = CUDAThreadEntry::ThreadLocal()->stream;
//   }
...
//   IdType* index_ptr = IsNullArray(index)? nullptr: index.Ptr<IdType>();
//   IdType* input_mapping_ptr = IsNullArray(input_mapping)? nullptr:
//   input_mapping.Ptr<IdType>(); IdType* output_mapping_ptr =
//   IsNullArray(output_mapping)? nullptr: output_mapping.Ptr<IdType>(); if
//   (grid.x > 0) {
//     _CSRRowWiseLoadSubtensorAlignedKernelV2<BLOCK_ROWS><<<grid, block, 0,
//     stream>>>(
//       feat_dim, size, index_ptr, input_mapping_ptr, output_mapping_ptr,
//       input_table.Ptr<DataType>(), output_table.Ptr<DataType>()
//     );
...
__global__ void _FeatTypePartKernel(int n_nodes, IdType *nodes,
                                    IdType *feat_pos_map, IdType *counter,
                                    IdType *part_ids, IdType *part_pos) {
  ...
  if (threadIdx.x < 2) {
    atomicAdd((unsigned long long *)(counter + threadIdx.x),
              local_count[threadIdx.x]);
  }
}

// part_ids, part_pos, part_offset
std::tuple<IdArray, IdArray, IdArray> GetFeatTypePartIds(IdArray nodes,
                                                         IdArray feat_pos_map) {
  IdArray part_ids =
      IdArray::Empty({nodes->shape[0]}, nodes->dtype, nodes->ctx);
  IdArray part_pos =
      IdArray::Empty({nodes->shape[0]}, nodes->dtype, nodes->ctx);
  ...
  if (n_blocks == 0)
    n_blocks = 1;
  auto *thr_entry = CUDAThreadEntry::ThreadLocal();
  _FeatTypePartKernel<<<n_blocks, n_threads, 0, thr_entry->stream>>>(
      nodes->shape[0], nodes.Ptr<IdType>(), feat_pos_map.Ptr<IdType>(),
      part_sizes.Ptr<IdType>(), part_ids.Ptr<IdType>(), part_pos.Ptr<IdType>());
